...
fn contract_receive_message<S: HasStateApi>(
    ...
    let sender_address: AccountAddress = match sender {
    ...
    match msg {
        Message::RequestTransfer(req_id, transfer_amount, target_account) => {
            ...
            let mut reserved_balance: Amount = Amount::zero();
            ...
            let balance: Amount = amount + host.self_balance();
            ensure!(
            ...
            let mut supporters: BTreeSet<AccountAddress> = BTreeSet::new();
            supporters.insert(sender_address);
            let new_request: TransferRequest = TransferRequest {
                transfer_amount,
                target_account,
                times_out_at: now
        ...
        Message::SupportTransfer(transfer_request_id, transfer_amount, target_account) => {
            let transfer: bool = {
                ...
                let mut matching_request: OccupiedEntry<u64, TransferRequest, S> = host
                    .state_mut()
                    .requests
...
mod tests {
    ...
    fn test_init() {
        // Setup context
        let account1: AccountAddress = AccountAddress([1u8; 32]);
        let account2: AccountAddress = AccountAddress([2u8; 32]);

        let mut account_holders: BTreeSet<AccountAddress> = BTreeSet::new();
        account_holders.insert(account1);
        account_holders.insert(account2);

        let parameter: InitParams = InitParams {
            account_holders,
            transfer_agreement_threshold: 2,
            transfer_request_ttl: TransferRequestTimeToLiveMilliseconds::from_millis(10),
        };
        let parameter_bytes: Vec<u8> = to_bytes(&parameter);

        let mut ctx: TestContext<TestInitOnlyData> = TestInitContext::empty();
        ctx.set_parameter(&parameter_bytes);

        let amount: Amount = Amount::from_micro_ccd(0);
        let mut state_builder: StateBuilder<TestStateApi> = TestStateBuilder::new();
        // call the init function
        let out: Result<State<TestStateApi>, InitError> =
            contract_init(&ctx, &mut state_builder, amount);

        // and inspect the result.
        let state: State<TestStateApi> = match out {
            Ok(s) => s,
    ...
    fn test_receive_request() {
        ...
        let account1: AccountAddress = AccountAddress([1u8; 32]);
        let account2: AccountAddress = AccountAddress([2u8; 32]);
        let target_account: AccountAddress = AccountAddress([3u8; 32]);

        let request_id: u64 = 0;
        let transfer_amount: Amount = Amount::from_micro_ccd(50);
        // Create Request with id 0, to transfer 50 to target_account
        let parameter: Message =
            Message::RequestTransfer(request_id, transfer_amount, target_account);
        let parameter_bytes: Vec<u8> = to_bytes(&parameter);
        ...
        let mut state_builder: StateBuilder<TestStateApi> = TestStateBuilder::new();
        let state: State<TestStateApi> = State {
            init_params,
            requests: state_builder.new_map(),
        };
        let mut host: TestHost<State<TestStateApi>> = TestHost::new(state, state_builder);
        host.set_self_balance(Amount::from_micro_ccd(0));

        let receive_amount: Amount = Amount::from_micro_ccd(100);
        // Execution
        let res: Result<(), ReceiveError> =
            contract_receive_message(&ctx, &mut host, receive_amount);
        ...
        let request: StateRef<TransferRequest> = host.state().requests.get(&request_id).unwrap();
    ...
    fn test_receive_support_no_transfer() {
        ...
        let account1: AccountAddress = AccountAddress([1u8; 32]);
        let account2: AccountAddress = AccountAddress([2u8; 32]);
        let account3: AccountAddress = AccountAddress([3u8; 32]);
        let target_account: AccountAddress = AccountAddress([3u8; 32]);

        let request_id: u64 = 0;
        let transfer_amount: Amount = Amount::from_micro_ccd(50);
        let parameter: Message =
            Message::SupportTransfer(request_id, transfer_amount, target_account);
        let parameter_bytes: Vec<u8> = to_bytes(&parameter);
        ...
        let mut state_builder: StateBuilder<TestStateApi> = TestStateBuilder::new();
        let mut requests: StateMap<u64, TransferRequest, TestStateApi> = state_builder.new_map();
        ...
        let receive_amount: Amount = Amount::from_micro_ccd(75);
    ...
    fn test_receive_duplicate_request() {
        ...
        let request_id_outdated: u64 = 0;
        let request_id_active: u64 = 1;
        let transfer_amount: Amount = Amount::from_micro_ccd(50);
        // Create Request with id 0, to transfer 50 to target_account
        let parameter: Message =
        ...
        let res: Result<(), ReceiveError> =
            host.with_rollback(|host| contract_receive_message(&ctx, host, Amount::zero()));
